(() => {
  ...
  const getConversationTurnUnit = (message, order) => {
    ...
    const messages = section
      ? $$(messageRoleSelector, section).filter((node) => node.closest?.('[data-testid^="conversation-turn-"]') === section)
      : [message];
  ...
  const normalizeHistoryDate = (value) => {
    ...
    if (typeof value === 'number' && Number.isFinite(value)) {
      ...
      return new Date(value < 10 ** 10 ? value * 1000 : value);
    }
    if (typeof value === 'string' && value.trim()) {
      const numeric = Number(value);
      if (Number.isFinite(numeric) && numeric <= 0) return null;
      const date = Number.isFinite(numeric)
        ? new Date(numeric < 10 ** 10 ? numeric * 1000 : numeric)
        : new Date(value);
  ...
  const buildConversationRecord = (payload, options = {}) => {
    ...
    return {
      ...
      update_time: updateTime || new Date(),
      last,
      model: String(
  ...
  const extractConversationRecords = (payload, fallbackId = '') => {
    ...
    const addRecord = (record) => {
      ...
      records.set(record.id, {
        ...
        update_time: record.update_time || current.update_time || new Date(),
